...
package com.wjc.jcdemolist.demo.CusImageview;
...
public class CusCircleView extends ImageView {
    ...
    private Drawable mBackgroundDrawable; // 当前背景 drawable
    private ColorStateList mBorderColor =
            ColorStateList.valueOf(CusCircleDrawable.DEFAULT_BORDER_COLOR);
    private float mBorderWidth = DEFAULT_BORDER_WIDTH;
    private ColorFilter mColorFilter = null;
    private boolean mColorMod = false;
    private Drawable mDrawable; // 当前 drawable
    ...
    public CusCircleView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }


    // 自定义声明与获取
    public CusCircleView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CusCircleView, defStyle, 0);

        int index = a.getInt(R.styleable.CusCircleView_android_scaleType, -1);
        if (index >= 0) {
        ...
        float cornerRadiusOverride =
                a.getDimensionPixelSize(R.styleable.CusCircleView_riv_corner_radius, -1);

        mCornerRadii[Corner.TOP_LEFT] =
                a.getDimensionPixelSize(R.styleable.CusCircleView_riv_corner_radius_top_left, -1);
        mCornerRadii[Corner.TOP_RIGHT] =
                a.getDimensionPixelSize(R.styleable.CusCircleView_riv_corner_radius_top_right, -1);
        mCornerRadii[Corner.BOTTOM_RIGHT] =
                a.getDimensionPixelSize(R.styleable.CusCircleView_riv_corner_radius_bottom_right, -1);
        mCornerRadii[Corner.BOTTOM_LEFT] =
                a.getDimensionPixelSize(R.styleable.CusCircleView_riv_corner_radius_bottom_left, -1);
        ...
        mBorderWidth = a.getDimensionPixelSize(R.styleable.CusCircleView_riv_border_width, -1);
        if (mBorderWidth < 0) {
            mBorderWidth = DEFAULT_BORDER_WIDTH;
        }

        mBorderColor = a.getColorStateList(R.styleable.CusCircleView_riv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(CusCircleDrawable.DEFAULT_BORDER_COLOR);
        }
        ...
        final int tileModeX =
                a.getInt(R.styleable.CusCircleView_riv_tile_mode_x, TILE_MODE_UNDEFINED);
        if (tileModeX != TILE_MODE_UNDEFINED) {
            setTileModeX(parseTileMode(tileModeX));
        }

        final int tileModeY =
                a.getInt(R.styleable.CusCircleView_riv_tile_mode_y, TILE_MODE_UNDEFINED);
    ...
    private static Shader.TileMode parseTileMode(int tileMode) {
        switch (tileMode) {
            case TILE_MODE_CLAMP:
                return Shader.TileMode.CLAMP;  // 边缘色来填充多余空间
            case TILE_MODE_REPEAT:
                return Shader.TileMode.REPEAT;  // 重复原图像来填充空间
    ...
    public void setImageDrawable(Drawable drawable) {
        mResource = 0;
        mDrawable = CusCircleDrawable.fromDrawable(drawable);
        updateDrawableAttrs();
    ...
    public void setImageBitmap(Bitmap bm) {
        mResource = 0;
        mDrawable = CusCircleDrawable.fromBitmap(bm);
        updateDrawableAttrs();
        super.setImageDrawable(mDrawable);
    }
...
//        }
    ...
    private Drawable resolveResource() {
        ...
        return CusCircleDrawable.fromDrawable(d);
    }
    ...
    private void updateBackgroundDrawableAttrs(boolean convert) {
        if (mMutateBackground) {
            if (convert) {
                mBackgroundDrawable = CusCircleDrawable.fromDrawable(mBackgroundDrawable);
            }
    ...
    private void updateAttrs(Drawable drawable) {
        ...
        if (drawable instanceof CusCircleDrawable) {
            ((CusCircleDrawable) drawable)   // 配置基本参数
            ...
            if (mCornerRadii != null) {  // 圆角
                ((CusCircleDrawable) drawable).setCornerRadius(
                        mCornerRadii[Corner.TOP_LEFT],
                        mCornerRadii[Corner.TOP_RIGHT],
                        mCornerRadii[Corner.BOTTOM_RIGHT],
                        mCornerRadii[Corner.BOTTOM_LEFT]);
  ...
  /*  @Override
    ...
    public void setBorderColor(ColorStateList colors) {
        ...
        mBorderColor =
                (colors != null) ? colors : ColorStateList.valueOf(CusCircleDrawable.DEFAULT_BORDER_COLOR);
        updateDrawableAttrs();
        updateBackgroundDrawableAttrs(false);
        if (mBorderWidth > 0) {
            invalidate();
